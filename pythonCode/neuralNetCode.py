...
def select_class_model(X, y, option, debug = 0, para_p = 'l2', para = 1):
	...
	elif option == 5: # random forest
		# adjust n_estimators and max_features
...
def cross_val(X, y, problem_type, option):
	...
	if problem_type: 
		reg = select_reg_model(X_train, y_train, option)
		test_score = reg.score(X_test, y_test, option)
		print(cross_val_score(reg, X, y, cv = 5))
	else:
		cls = select_class_model(X_train, y_train, option)
		test_score = cls.score(X_test, y_test, option)
